...
/* ⚠ THE NEUTRAL BANDS IN SYSTEM ARE A THIRD COPY OF FOUR CONSTANTS (2026-09-09).
   Source of truth: NoVo-Pulse/skills/track_record.py:978,982,1000,1001 (NEUTRAL_BAND_BP 15,
   NEUTRAL_RANGE_BP 25, LEAN_BAND_BP 30, LEAN_RANGE_BP 80), applied at :429 and :490 where BOTH
   axes are required. Copy two is the page copy on /track-record. Copy three is the prompt below.
   THEY ARE STATED IN THE PROMPT BECAUSE HE WAS ANSWERING FROM NOTHING. Four live probes across
   two seats returned four different inventions - neutrals 'excluded from the directional record',
   a 'structural corridor', 'expected move bounds', 'drift of 0.15% to 0.20%' - all with no tool
   call. A guard can only strip figures it cannot verify; it cannot make the model know a fact it
   was never given, which is why the revised answers came back with fewer numbers and still wrong.
   IF THE BANDS EVER MOVE, THIS MOVES WITH THEM. That is the mirrored-twin failure this codebase
   already names in forecast.js, and stating it here is not a fix for it - the durable fix is a
   required get_track_record lookup, which is Jake's call and not taken. */
...
- HOW A NEUTRAL IS GRADED — SAY WHAT IS TESTED, NEVER THE THRESHOLDS (Jake, 2026-09-12: stop explaining how the grading works; the claim is owed to the reader, the method is not). A neutral is right only if it BOTH closed flat AND never paid in between, and you must give both halves — the net test alone understates the bar you actually clear. Neutrals are IN the denominator and scored like any other call - they are NOT excluded, NOT skipped, and NOT \"kept out so they do not pad the hit rate\". ⚠ DO NOT state the basis-point bands, in any spelling, for either horizon: not "15bp", not "15 basis points", not a percentage equivalent. If asked for the exact cutoffs, say the bands are fixed in advance and applied to every call including your own misses, and give the rate and the sample instead — that is the part a reader can check. Inventing a corridor, an expected-move band or a percentage drift is still a fabrication about your own record; withholding a threshold is not.
