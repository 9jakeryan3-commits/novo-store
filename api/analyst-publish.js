...
function _levelsTable(levels) {
  ...
  const rows = (items, c) => (items.length ? items : [null]).map(l => l
    ? `<tr><td style="padding:6px 12px;color:#9fb6d1;font-size:13px;border-top:1px solid #2e3036;">${esc(l.label)}</td><td style="padding:6px 12px;text-align:right;font-weight:700;color:${c};font-size:13px;border-top:1px solid #2e3036;">${fmt(l.price)}</td></tr>`
...
async function _promotePublicLevels(state) {
  ...
  // Live-state save — the engine POSTs the current SPY/QQQ/IWM dealer state (secret-auth) for the members
  ...
  if (levels.length) {
    ...
    const rowsHtml = (items, col) => (items.length ? items : [null]).map(l =>
      l ? `<tr><td style="padding:6px 12px;color:#9fb6d1;font-size:13px;border-top:1px solid #2e3036;">${esc(l.label)}</td><td style="padding:6px 12px;text-align:right;font-weight:700;color:${col};font-size:13px;border-top:1px solid #2e3036;">${fmt(l.price)}</td></tr>`
        : `<tr><td style="padding:6px 12px;color:#6f8bab;font-size:13px;">&mdash;</td><td></td></tr>`).join('');
    const colCell = (t, items, col) =>
      `<td style="vertical-align:top;width:50%;padding:0 5px;"><div style="border:1px solid #2e3036;border-top:2px solid ${col};border-radius:8px;overflow:hidden;"><div style="background:#26272c;padding:7px 12px;font-size:10.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:${col};">${t}</div><table style="width:100%;border-collapse:collapse;">${rowsHtml(items,col)}</table></div></td>`;
  ...
  const buildUpsell = (u) => u === 'both'
    ...
    ? '<div style="font-size:14px;color:#eaf3ff;font-weight:700;margin-bottom:8px;">Ready for more than the taste?</div>' +
      '<div style="font-size:13.5px;color:#9fb6d1;line-height:1.55;margin-bottom:11px;"><b style="color:#eaf3ff">NoVo Analyst</b> &mdash; the daily <b style="color:#eaf3ff">Open</b> &amp; <b style="color:#eaf3ff">Close</b> desk notes, real-time level-break alerts, and a <b style="color:#eaf3ff">live dealer dashboard</b>, $129/mo. <a href="https://novo-options.trade/analyst" style="color:#34d399;font-weight:700;text-decoration:none;">Get NoVo Analyst &rarr;</a></div>' +
      '<div style="font-size:13.5px;color:#9fb6d1;line-height:1.55;"><b style="color:#eaf3ff">NoVo Trader</b> &mdash; the same read, streamed live on a candle chart, with the levels drawn on the tape as they move. <a href="https://novo-options.trade" style="color:#34d399;font-weight:700;text-decoration:none;">See NoVo Trader &rarr;</a></div>'
    : u === 'analyst'
    ...
    : '<div style="font-size:14px;color:#eaf3ff;font-weight:700;margin-bottom:4px;">Want it raw &amp; live?</div>' +
      '<div style="font-size:13.5px;color:#9fb6d1;line-height:1.55;">This is the read. <b style="color:#eaf3ff">NoVo Trader</b> is the same read, streamed live on a candle chart, with an hourly structural audit beside it. <a href="https://novo-options.trade" style="color:#34d399;font-weight:700;text-decoration:none;">See NoVo Trader &rarr;</a></div>';
  ...
  // Discord, so they can unsubscribe from email anytime without losing a thing. Broadcast merge tag → real link.
  const UNSUB_NOTE = '<div style="margin-top:24px;border:1px solid #2e3036;border-left:3px solid #22d3ee;border-radius:8px;padding:15px 17px;background:rgba(34,211,238,0.06);">' +
    '<div style="font-size:13.5px;color:#c2d2e6;line-height:1.6;"><b style="color:#eaf3ff;">Don&rsquo;t need these in your inbox?</b><br>' +
    'Everything in this email is live in your <b style="color:#22d3ee;">dealer dashboard</b> and the <b style="color:#7f8cff;">Analyst Discord</b> &mdash; email is just a backup for when you&rsquo;re away from them. ' +
    'Once you&rsquo;re set up there, <a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#34d399;font-weight:800;text-decoration:underline;">click here to unsubscribe from emails</a> anytime. ' +
    'Your subscription, dashboard, and Discord access stay exactly the same.</div></div>';
  const renderBody = (upsellHtml, liveCta, bottomNote) => html || (
    '<div style="margin:0;padding:0;background:#101013;">' +
      '<div style="max-width:600px;margin:0 auto;padding:24px 12px;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">' +
        '<div style="background:#17181b;border:1px solid #2e3036;border-bottom:0;border-radius:12px 12px 0 0;padding:22px 24px;text-align:center;">' +
          '<img src="https://novo-options.trade/novo-logo-light.png?v=6" alt="NoVo Options Trading" height="30" style="height:30px;width:auto;display:inline-block;border:0;">' +
        ...
        '<div style="background:#1c1d21;border:1px solid #2e3036;border-top:0;border-radius:0 0 12px 12px;padding:28px 28px 24px;">' +
          (liveCta || '') +
          (chartUrl ? `<img src="${chartUrl}" width="552" style="width:100%;max-width:552px;height:auto;border-radius:8px;border:1px solid #2e3036;display:block;margin:0 0 20px;" alt="SPY session chart — levels &amp; structure">` : '') +
          ...
          '<div style="margin-top:24px;border:1px solid #2e3036;border-radius:8px;padding:14px 16px;background:#212227;">' +
        ...
        '</div>' +
        '<div style="text-align:center;font-size:11px;color:#6f8bab;padding:14px 8px;">You are subscribed to NoVo email updates. <a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#6f8bab;text-decoration:underline;">Unsubscribe</a>' + postalHtml() + '</div>' +
      '</div>' +
    '</div>'
  );
