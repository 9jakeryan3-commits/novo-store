...
_LICENSE_SECRET   = os.environ.get("NOVO_LICENSE_SECRET", "")
_ZIP_URL          = os.environ.get("NOVO_ZIP_URL", "")
_FROM_EMAIL       = os.environ.get("FROM_EMAIL", "NoVo <orders@novo-trading.com>")
_WEBHOOK_SECRET   = os.environ.get("STRIPE_WEBHOOK_SECRET", "")


def generate_key() -> str:
    token = secrets.token_hex(8).upper()
    sig = hmac.new(_LICENSE_SECRET.encode(), token.encode(), hashlib.sha256).hexdigest()[:8].upper()
    return f"NOVO-{token[:8]}-{token[8:]}-{sig}"


def send_email(to_email: str, license_key: str):
    html = f"""<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<style>
  ...
  .divider {{ border:none; border-top:1px solid #1b2e4e; margin:28px 0; }}
...
<div class="wrap">
  ...
  <a href="{_ZIP_URL}" class="btn">Download NoVo v.fast</a>
  ...
  <p>Full walkthrough and troubleshooting is in <strong>START&amp;TROUBLESHOOT.pdf</strong> inside the zip. The presentation guide explains how the system thinks and makes decisions.</p>
  <p>Start in paper mode (default). Watch it run for a few sessions before switching to live.</p>
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            event = stripe.Webhook.construct_event(body, sig_header, _WEBHOOK_SECRET)
