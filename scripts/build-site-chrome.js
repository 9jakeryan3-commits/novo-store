...
/* ── head: the stylesheet links and the one inline <style> block, IN SOURCE ORDER ───────────────
   ⚠ ORDER IS THE PAYLOAD, NOT JUST THE BYTES (2026-09-12, Jake on /daily-strike: "header still a
   disater ... look at the 1500 other headers that fucking work ... it is the same we have done it
   1500 times").

   This used to emit `links + style`, which is not how any real page is built. In analyst.html the
   inline <style> closes at 45020 and the polish.css link follows at 45386 — polish.css is LAST and
   therefore WINS. Concatenating links-then-style silently inverted that cascade on every
   server-rendered page, so the inline block won instead, and the two rules polish.css uses to
   assemble the header lost at equal specificity:

     .nav-actions { display: contents; }      <- the nav's links/Plans/account become flex items of
                                                 .nav-inner and honour their own `order:` values
     .nav-inner, .ticker-row { max-width: 1560px; }   <- the wide shell the rest of the site uses

   Measured on the live pages at 1440px, same CSS bytes on both:
     /analyst       .nav-inner max-width 1560px, .nav-actions width 0 (contents), nav = 1 row
     /daily-strike  .nav-inner max-width 1200px, .nav-actions a 1003px box, nav = 2 ROWS
   — the links collided inside that box, Plans and the account icon landed left of them, and the
   logo was pushed onto a second row. It did not look unstyled, it looked REARRANGED, which is why
   it read as a different site rather than a broken one.

   So the head is rebuilt in the order the source file declares, by position, rather than by
   category. A page that reorders its own head is then reproduced faithfully instead of guessed at. */
const headEnd = html.indexOf('</head>');
if (headEnd < 0) throw new Error('build-site-chrome: no </head> in ' + SOURCE);
const headHtml = html.slice(0, headEnd);

const cssNodes = [];
for (const m of headHtml.matchAll(/<link[^>]*rel="stylesheet"[^>]*>/g)) cssNodes.push({ at: m.index, html: m[0] });
for (const m of headHtml.matchAll(/<style[^>]*>[\s\S]*?<\/style>/g)) cssNodes.push({ at: m.index, html: m[0] });
cssNodes.sort((a, b) => a.at - b.at);

const links = cssNodes.filter((n) => n.html.startsWith('<link')).map((n) => n.html).join('');
const styleM = cssNodes.filter((n) => n.html.startsWith('<style')).map((n) => n.html);
if (!links) throw new Error('build-site-chrome: no stylesheet links found');
if (!styleM.length) throw new Error('build-site-chrome: no inline <style> block found');

const head = cssNodes.map((n) => n.html).join('');

/* Generation-time proof the cascade still reads the way the static pages do. Keyed on the actual
   consequence — polish.css must be able to beat the inline block — so it fails at build if anyone
   reorders the head again, instead of shipping a rearranged header to readers. */
const polishAt = head.indexOf('polish.css');
const lastStyleEnd = head.lastIndexOf('</style>');
if (polishAt < 0) throw new Error('build-site-chrome: polish.css is not linked in the head');
if (polishAt < lastStyleEnd) {
  throw new Error('build-site-chrome: polish.css is emitted BEFORE the inline <style>, so the inline '
    + 'block wins — the header loses .nav-actions{display:contents} and the 1560px shell, and ships '
    + 'as two rows with colliding links. Emit the head in source order.');
}
